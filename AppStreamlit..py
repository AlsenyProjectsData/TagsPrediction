...
import joblib
##
model_rf = joblib.load(open("TagsPredicts.pkl", "rb"))
## Fct
def predict_tags(docx):
    results = model_rf.predict([docx])
    return results[0]
    
def predict_tags_label(docx):
    results = model_rf.predict([docx])
    tag =['.net','asp.net', 'c#','c++','java','javascript','php','python','sql','sql-server']
    df = pd.DataFrame({"id": list(results[0]), "label": tag})
    pred = df[df["id"]==1]
    return pred["label"]
...
## Apply Fct
           prediction = predict_tags_label(news_text)
