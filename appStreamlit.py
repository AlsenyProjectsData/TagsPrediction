...
def main():
    ...
    if choice == "Home":
        st.markdown("DETAILS: Dans cette application, la prédiction correspond à une colonne de dix valeurs qui indiquent si un tag a été prédit pour le texte donné (valeur 1) ou non (valeur 0). Chaque valeur correspond à un tag dans l'ordre suivant : .net, asp.net, c#, c++, java, javascript, php, python, sql, sql-server")
